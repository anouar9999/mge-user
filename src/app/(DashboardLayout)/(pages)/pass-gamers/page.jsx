...
const PassCard = ({ 
  ...
  return (
    <>
      <div 
        className="group relative w-full h-full font-pilot overflow-hidden bg-[#0a111a] border border-[#1f2731]/50 hover:border-primary/50 transition-all duration-500 shadow-lg hover:shadow-primary/10 cursor-pointer"
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
        onClick={handleMoreDetails}
      >
        {/* Diagonal split design with animated shapes */}
        <div className="absolute top-0 right-0 w-48 h-48 -mr-20 -mt-20 bg-primary/10 rounded-full transform rotate-45 transition-all duration-700 group-hover:rotate-90 group-hover:scale-150 group-hover:bg-primary/20 z-0"></div>
        <div className="absolute bottom-0 left-0 w-32 h-32 -ml-10 -mb-10 bg-primary/5 rounded-full transform rotate-45 transition-all duration-700 group-hover:rotate-90 group-hover:scale-150 group-hover:bg-primary/10 z-0"></div>
